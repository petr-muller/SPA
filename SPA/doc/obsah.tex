...
Example 2 \emph{with} pointers doesn't generate any warning. We used Clang version 3.5.0 for this test.
...
\For {declRefExpr in declRefExprs}
    \State node = declRefExpr
    \While{node.type != functionDecl}
        \If{nodeImpliesSideEffect(node)}
            \State tagNodeAndAllPredecessors(node, declRefExpr)
            \Break
        \ElsIf{nodePreservesLvalue(node)}
