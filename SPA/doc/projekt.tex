...
\projectinfo{
  ...
  %Abstract
  abstract.cs={Práce se zabývá detekcí určitého druhu nedefinovaného chování v programech v jazyce C. Zaméřujeme se na nedefinované chování týkájící se sequence pointů a vedlejších efektů, s důrazem na funkčnost i v případě přístupu k paměti přes ukazatel nebo volání funkce. K tomu je použita statická analýza. Je proveden teoretický rozbor, vysvětlení pojmů jako nedefinované chování nebo vedlejší efekt. Dále je vysvětlena nebezpečnost konstrukcí vedoucích k tomuto chování. Nakonec je navržen nástroj pro automatizovanou detekci a popsány principy jeho funkce. Práce obsahuje příklady nebezpečných konstrukcí, na některých z nich jsou demonstrovány funkce vytvořeného nástroje.}, % abstrakt v ceskem jazyce
  abstract.en={This thesis engages in detection of certain kind of undefined behavior in the C language programs. We focus on undefined behavior related to sequence points and side effects, stressing its functionality even in cases like accessing the memory via a pointer or function calling. Static analysis is used for this purpose. A theoretical analysis is done and terms like undefined behavior or side effect are explained. Furthermore, explanation of constructs leading to this behavior is done. Finally, a tool for automated detection is proposed and principles of its function shown. The thesis contains examples of dangerous constructs, functions of the created tool are demonstrated on some of them.}, % abstrakt v anglickem jazyce
  %Prohlaseni
